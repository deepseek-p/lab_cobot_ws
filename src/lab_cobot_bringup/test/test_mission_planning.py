...
def test_pick_step_tuck_failure_blocks_task_before_navigation():
    # 收臂失败必须阻断携物导航,避免伸展位形横扫工位。
    events = []
...
def test_station_a_uses_original_goal_with_handoff():
    goals = navigation_goals_for_station("station_a")

    assert [goal[0] for goal in goals] == ["station_a"]
    assert goals[0][1] == get_waypoint("station_a")
    assert goals[0][2] == "station_a"


def test_inspection_zone_uses_south_corridor_entry_before_station():
    goals = navigation_goals_for_station("inspection_zone")

    assert [goal[0] for goal in goals] == [
        "inspection_zone_south_corridor_entry",
        "inspection_zone",
    ]
...
def test_aging_zone_uses_east_corridor_before_staging():
    ...
    assert goals[2][2] == "aging_zone"
    assert goals[0][1]["x"] == goals[1][1]["x"]
    assert goals[1][1]["y"] == pytest.approx(2.97)
    assert goals[2][1] == get_waypoint("aging_zone")
