...
def test_station_safety_uses_map_pose_when_map_and_odom_are_offset():
    ...
    node.retreat_pub = type("Pub", (), {"publish": lambda self, _msg: None})()
...
def test_detect_state_uses_wrist_hit_and_pick_consumes_unified_cache():
    ...
    node._tuck_arm_for_transit = lambda: True
...
def test_nav_to_place_uses_nav2_before_local_place_docking():
    ...
    node._log_transit_state = lambda start_navigation: events.append(
        ("transit", start_navigation)
    )
...
def test_handoff_distances_do_not_allow_far_coarse_stop():
    assert mission_node.PICK_NAV_HANDOFF_MAX_STATION_DISTANCE <= 0.35
    assert mission_node.PLACE_NAV_HANDOFF_MAX_DISTANCE <= 0.30
...
def test_place_navigation_can_handoff_when_base_is_close_enough_for_local_docking():
    place_navigation_handoff_ready = _policy("place_navigation_handoff_ready")
    station_x, station_y, _station_yaw = mission_node._station_base_pose("station_b")
    base_pose = (station_x, station_y - 0.20, math.radians(147.0))
