...
def test_default_place_pose_targets_reachable_station_b_table_front():
    ...
    assert (-0.50 <= map_x <= 1.10) or (-2.4 <= map_x <= -1.6)
    assert (
        STATION_B_SAFE_DROP_FRONT_Y <= map_y <= table_mid_y
        or STATION_B_SAFE_DROP_FRONT_Y <= map_y <= safe_drop_back_y
    )
    assert map_y - STATION_B_SAFE_DROP_FRONT_Y >= minimum_nominal_front_margin
    assert (
        DEFAULT_PLACE_POSE[2] == pytest.approx(0.650)
        or DEFAULT_PLACE_POSE[2] == pytest.approx(0.725)
    )
    ...
    assert drop_height <= vision_z_error_band or drop_height <= 0.08
