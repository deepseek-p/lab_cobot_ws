...
def test_contact_gripper_never_falls_back_to_attach_bridge_after_timeout():
    fake_node = FakeNode(
    ...
    assert not driver.acquire_object()
    assert "/gripper/attach/aruco_sample" not in fake_node.empty_topics
    assert any(
        "contact attach timed out" in message
