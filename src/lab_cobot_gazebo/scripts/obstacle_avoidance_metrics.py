...
LIDAR_MAX_RANGE = 3.0
GHOST_RADIUS = 0.35
SAFETY_WARN_DIST = 1.60
SAFETY_CRITICAL_DIST = 1.20
SAFETY_EVADE_SPEED = 0.30           # short active nudge only inside critical clearance
SAFETY_EVADE_CRITICAL_SPEED = 0.42  # visible evasion before 0.3m contact threshold
SAFETY_EVADE_HYSTERESIS = 0.35       # extra clearance before stopping evasion
NAV2_IDLE_TIMEOUT = 0.5             # seconds without non-zero cmd_vel_nav
COSTMAP_LETHAL = 100
COSTMAP_OCCUPIED = 90
DEVIATION_ANGLE_THRESH = 0.2618  # 15 deg in rad
...
class AvoidanceMetrics(Node):
    ...
    def _distance_to_ghost(self) -> Optional[float]:
        ...
        return math.hypot(dx, dy) - GHOST_RADIUS
    ...
    def _publish_evasion(self, speed: float) -> None:
        """Publish velocity directly away from the ghost person."""
        robot = self._get_robot_pose()
        ghost = self._ghost_pose
        if robot is None or ghost is None:
            return
        ...
        if dist < 1e-6:
            return
        # cmd_vel_safety is expressed in base_footprint, not in map.  Publishing
        # the map-frame vector directly can move a rotated robot toward the actor.
        yaw = robot[2]
        away_x = dx / dist
        away_y = dy / dist
        msg = Twist()
        msg.linear.x = speed * (math.cos(yaw) * away_x + math.sin(yaw) * away_y)
        msg.linear.y = speed * (-math.sin(yaw) * away_x + math.cos(yaw) * away_y)
    ...
    def _handle_safety_evasion(self, dist: Optional[float]) -> None:
        ...
        if dist is None or not self._safety_override_allowed():
            if self._evading:
    ...
    def _evaluate(self) -> None:
        ...
        # --- active safety evasion (always runs) ---
