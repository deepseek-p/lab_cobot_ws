...
def _world_include_model_dir(entity_name):
    world = ElementTree.parse(GAZEBO / "worlds" / "lab.world").getroot()
    for include in world.findall(".//include"):
        if include.findtext("name") == entity_name:
            uri = include.findtext("uri")
            assert uri.startswith("model://")
            return uri.removeprefix("model://")
    raise AssertionError(f"missing world include {entity_name}")


def test_urdf_grasp_candidate_list_keeps_sample_tools_and_tubes():
    plugin = _plugin(_robot_xml(), "lab_cobot_grasp_fix")
    ...
    assert candidates[0] == "aruco_sample"
...
def test_grasp_candidates_are_dynamic_models_with_link_named_link():
    ...
    for name in candidates:
        model_dir = (
            name if name == "aruco_sample" else _world_include_model_dir(name)
        )
        model_file = GAZEBO / "models" / model_dir / "model.sdf"
        root = ElementTree.parse(model_file).getroot()
        model = root.find(f".//model[@name='{model_dir}']")
