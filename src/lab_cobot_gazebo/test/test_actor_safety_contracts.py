...
def test_safety_evasion_only_intervenes_near_contact_threshold():
    source = (GAZEBO / "scripts" / "obstacle_avoidance_metrics.py").read_text(
        encoding="utf-8"
    )

    assert "SAFETY_WARN_DIST = 1.60" in source
    assert "SAFETY_CRITICAL_DIST = 1.20" in source
    assert "SAFETY_EVADE_CRITICAL_SPEED = 0.42" in source
...
def test_metrics_gates_safety_evasion_to_navigation_states():
    source = (GAZEBO / "scripts" / "obstacle_avoidance_metrics.py").read_text(
        encoding="utf-8"
    )

    assert '"/task/status"' in source
    assert "def _safety_override_allowed" in source
    assert 'status.startswith("NAV_TO_")' in source
    assert 'status.startswith("RETURN_HOME")' in source


def test_actor_shadow_default_route_crosses_t1_navigation_legs():
    source = (GAZEBO / "scripts" / "actor_collision_shadow.py").read_text(
        encoding="utf-8"
    )

    assert "(148.0, 6.00, -3.60" in source
    assert "(155.0, 1.00, -2.60" in source
    assert "(300.0, -6.00, 4.00" in source
