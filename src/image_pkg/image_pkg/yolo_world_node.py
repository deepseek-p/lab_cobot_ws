...
class YoloWorldNode(Node):
    """Run the fixed eight-class detector without blocking ROS callbacks."""
    ...
    def __init__(self):
        ...
        defaults = {
            ...
            "model_path": "~/lab_cobot_models/lab_cobot_eight_class.pt",
            "target_classes": [
            ...
            # 0.08--0.28 range, so a conventional 0.25 threshold is too high.
    ...
    def callback(self, msg):
        """Keep only the newest camera frame for the inference worker."""
    ...
    def _add_aruco_fallback(self, image, detections):
        """Add deterministic ArUco boxes for the marked sample class."""
        ...
        if marker_ids is None:
            # The camera sees the 70 mm marker at a small pixel size.
    ...
    def destroy_node(self):
        """Stop the inference worker before destroying the ROS node."""
        with self._condition:
            self._stopping = True
...
def main(args=None):
    """Run the fixed eight-class detector node."""
