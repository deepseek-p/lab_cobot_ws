...
该流程不是只把底盘开到工位。`station_cruise` 会在每个工位执行：安全到站、底盘停止、机械臂抬起、按该工位物品清单逐个生成腕部相机观察位、目标居中、YOLO 检测、RGB-D 三维转换、多视角确认、机械臂收拢，然后才前往下一工位。

首先启动不带搬运任务节点的完整仿真和视觉系统，避免巡检节点与抓取任务节点同时控制底盘或机械臂：
...
```bash
ros2 run image_pkg station_cruise --ros-args \
  -p dwell_seconds:=5.0 \
  -p require_station_detection:=true \
  -p require_centered_yolo_before_statistics:=true \
  -p per_object_observe_seconds:=8.0 \
  -p minimum_confirmed_views:=3
```

`station_cruise` 的默认路线就是上述四个工位，不需要再发送 `/task/instruction`。观察位可以利用 Gazebo 实体位置引导相机朝向，但 `use_truth_pose:=false` 保证 YOLO/RGB-D 输出不会被 Gazebo 真值替代。
...
source /opt/ros/humble/setup.bash
source install/setup.bash
ros2 topic echo /image_pkg/cruise/status
```
...
```text
ARRIVED:<工位>
OBSERVING_FINE:<工位>:<类别>:<Gazebo实体>:view=<序号>
PIPELINE_COUNTS:<工位>:<类别>:...:pose_ready=<数量>
MULTIVIEW_CONFIRMED:<工位>:<类别>:views=<数量>
OBSERVATION_CONFIRMED:<工位>
DONE:home
```

只有某物体得到 `MULTIVIEW_CONFIRMED`，才表示该物体已经完成腕部相机二维识别和三维定位；只有四个工位全部处理并返回 `home` 后出现 `DONE:home`，整次巡检才完成。

不要使用下面这条普通导航指令替代视觉巡检：
...
该指令属于任务编排器的导航路线，只保证依次到站，不会执行 `station_cruise` 的逐物体腕部观察和多视角视觉确认。
