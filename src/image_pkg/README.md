...
`image_pkg` 是本工作空间的视觉包。它使用项目自训练的固定八类 YOLO 模型取得二维检测框，再与**和 RGB 图像逐像素对齐的有组织点云**关联，计算目标三维位置，并通过 TF 转换到 `base_link`，供抓取或任务模块使用。

默认链路不依赖 Gazebo 真值或模板模型：Gazebo `ModelStates` 仅用于记录误差评测，绝不会把真值发送给抓取模块。

## 最新闭环验证（2026-08-07）

已在无 GUI Gazebo 中完成 ArUco 样件的视觉抓取与搬运闭环：A 工位导航和停靠 → 腕部相机初检 → 位姿转换到 `base_link` → 靠近目标 → 腕部二次观测修正 → 触觉夹爪附着 → 抬升、转运至 B 工位、释放并返回 home。最终任务状态为 `DONE`，夹爪事件依次为 `attached aruco_sample`、`release accepted`。

为使仿真抓取插件能够选择该工件，`lab_cobot_gazebo/models/aruco_sample/model.sdf` 已将样件设为动态模型（`<static>false</static>`）；静态模型会被抓取插件视为场景物而跳过。完整执行和误差口径见 [`lighting_test_report.md`](lighting_test_report.md)。

## 功能与边界

- 使用 `models/best.pt` 识别八个已训练类别，输出类别、置信度和检测框。
...
- 可在 `yolo` 与 `aruco` 两种定位来源之间选择，并统一输出到一个目标位姿话题；YOLO 模式只发布配置的 `target_label` 的最佳有效三维位姿，避免多目标误抓。
...
| `image_pkg/models/best.pt` | 项目自训练的八类 YOLO 权重；构建时会安装到 ROS 包的共享目录。 |
| `image_pkg/config/data.yaml` | 训练数据集记录文件，仅供训练/复现实验参考；不由部署节点读取。保留其中原始 Windows `path` 不影响 ROS 推理。 |
| `image_pkg/yolo_world_detector.py` | ROS 无关的固定类别 YOLO 封装；定义不可变的 `Detection`（类别、置信度、`xyxy` 框），负责解析安装后的权重路径、选择 CPU/CUDA、执行推理。 |
...
| `lighting_test_report.md` | 六种光照/遮挡工况与 ArUco 视觉抓取闭环的实际测试记录。 |

文件名带 `:Zone.Identifier` 的条目是从 Windows 文件系统带入的来源标记，不参与 ROS 构建和运行。

`test/` 曾仅包含单元测试和代码风格检查，不参与节点启动或运行时功能，已按项目清理要求移除。
...
7. 为每个有效目标发布标准三维检测、位姿以及 JSON 兼容输出；仅将 `target_label` 匹配且置信度最高的有效目标发布至单目标话题 `/perception/target_pose`。
...
| `model_path` | `models/best.pt` | 自训练 YOLO 权重。相对路径自动从已安装的 `image_pkg` 包目录解析。 |
| `target_classes` | 八个项目语义名称 | 下游抓取/评测允许的统一类别集合。 |
| `class_label_aliases` | `[]` | 保留新 `best.pt` 的原始训练类别名，不使用旧语义别名。 |
...
| `target_label` | `aruco_sample` | `pose_source=yolo` 时单目标输出所选择的项目语义标签。设为空字符串时才退回选择置信度最高的有效三维目标。 |
...
运行依赖包括 ROS 2 Humble、`cv_bridge`、`sensor_msgs_py`、`vision_msgs`、TF2、NumPy、OpenCV、PyTorch 和 Ultralytics YOLO。

## 训练模型与类别映射

推理只需要 `models/best.pt`；不需要训练集图像、标签，也不读取 `config/data.yaml`。因此 `data.yaml` 内原训练机的 Windows 路径无需、也不应为了部署改成一个并不存在的 ROS 路径。只有准备在本机继续训练/验证时，才复制完整数据集并把其中的 `path` 改为该数据集根目录。

`best.pt` 的原始类别顺序来自 `config/data.yaml`，部署输出会统一映射如下：

| 权重类别名 | 发布/抓取类别名 |
| --- | --- |
| `material_spare_igbt` | `material_spare_igbt` |
| `aruco_sample` | `aruco_sample` |
| `material_grease_can` | `material_grease_can` |
...
节点不再使用 YOLO-World 文本提示词，也不再使用按颜色猜测物体的 Gazebo 兜底逻辑；这可避免把非这八类的物体发布为可抓取目标。ArUco 检测仅作为 `aruco_sample` 的补充定位来源。
...
- **YOLO 模型加载失败**：确认 `image_pkg/models/best.pt` 存在，且在修改后已重新构建并 `source install/setup.bash`；确认已安装 Ultralytics。没有 CUDA 环境时使用 `device:=cpu`。

## V3 与 V4 包内能力边界

`lighting_benchmark` 的汇总现在只把具有当前三维误差字段的结果作为 V3 证据，并检查六个预期工况是否各自达到至少 100 个有效三维定位。旧格式结果会在报告中明确标为未纳入，不能被误作验收通过。该工具不能替代真实六工况运行，也不会把漏检伪装成成功。
...
它发布 `/perception/device_states` 的 JSON，字段包括 `device_id`、`state`（`running`、`idle`、`fault` 或 `unknown`）、颜色、置信度和各颜色像素数。使用前必须按相机画面配置 `indicator_state.roi`。当画面中多盏灯同时亮起、颜色没有明显主导时，节点会保守发布 `unknown`；设备模型需要在仿真侧实现单一灯态切换后，才能完成 V4 的端到端验收。
