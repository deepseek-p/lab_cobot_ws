...
description: >
  Deep-dive ROS 2 engineering reference for specialist topics. TRIGGER when the user:
  implements ros2_control hardware interface plugins or controllers, tunes real-time
  behavior (PREEMPT_RT, executor jitter, memory locking, cyclictest), builds MoveIt 2
  task pipelines (MTC, Servo, pick_ik, planning scene), debugs distro-specific API
  breaking changes (Humble/Jazzy/Kilted migration matrices), designs software e-stop
  chains (heartbeat + DEADLINE QoS, twist_mux arbitration), or needs QoS/DDS tuning
  beyond defaults (shared memory transport, DEADLINE/LIFESPAN semantics).
  DO NOT TRIGGER for general ROS 2 development — node/package scaffolding, launch files,
  colcon builds, basic Nav2/costmap configuration are covered by the ros2 skill.
---
...
| Software e-stop, heartbeat QoS, twist_mux, safety chain | `references/safety-estop.md` |
